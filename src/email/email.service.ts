...
export class EmailService {
    ...
    constructor(private configService: ConfigService) {
        ...
        this.fromEmail = this.configService.get<string>('EMAIL_FROM') || 'LEXA IA <noreply@lexa.com>';
    }

    async sendPasswordResetEmail(email: string, resetLink: string, userName: string = 'Usuário'): Promise<{ success: boolean; error?: string }> {
        try {
            // Added a check for this.resend to prevent runtime errors if API key is missing
            if (!this.resend) {
